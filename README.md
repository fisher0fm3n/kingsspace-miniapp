...
| Profile | `/profile` | History / Liked / Subscriptions tabs, sign out. |
| Auth | `/login`, `/auth/callback` | Username/password login **and** KingsChat OAuth (implicit flow → server-side token exchange). |
| Upload | `/upload` | Auth-gated create entry point. |
...
- **KingsChat login** uses the implicit OAuth flow against
  `accounts.kingsch.at` with client id `com.kingschat` (from the RN app). The
  web redirect URI is `<origin>/auth/callback`; that origin must be registered
  with KingsChat for the flow to complete in production. Username/password
  login works out of the box.
- **Video upload** is intentionally a stub — the RN app relies on native media
  processing / thumbnail generation that's out of scope for this web mini app.
